...
function cdr3lines() {
    viewer.clear();	
    var tcr = structure.select({cnames: 'DE'});
    viewer.cartoon('tcr', tcr, { color: pv.color.byChain() });
...
function cartoon() {
    viewer.clear();
    var mhca = structure.select({cnames: 'A'});
    viewer.cartoon('mhca', mhca, { color: pv.color.uniform('darkgreen')});
    var mhcb = structure.select({cnames: 'B'});
    viewer.cartoon('mhcb', mhcb, { color: pv.color.uniform('ligntgreen')});
    var pep = structure.select({cnames: 'C'});
    viewer.cartoon('pep', pep, { color: pv.color.uniform('magenta')});
    var tcra = structure.select({cnames: 'D'});
    viewer.cartoon('tcra', tcra, { color: pv.color.uniform('red')});
    var tcrb = structure.select({cnames: 'E'});
    viewer.cartoon('tcrb', tcrb, { color: pv.color.uniform('blue')});
}
