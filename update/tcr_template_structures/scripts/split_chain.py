...
myPath = "/home/rg/Fv/"
listfiles = glob.glob1(myPath,"*-*.pdb")
...
for infile in listfiles:
    ...
    if inpose.empty():
        print infile, "ignored"
        continue
    lpose = rosetta.Pose()
    hpose = rosetta.Pose()
    lpose = inpose.split_by_chain(1)
    hpose = inpose.split_by_chain(2)
    bname = os.path.splitext(os.path.basename(infile))[0]
    lfile = bname[:4]+"_"+bname[4]+"_L_aho.pdb"
    hfile = bname[6:10]+"_"+bname[10]+"_H_aho.pdb"
    lpose.dump_pdb(lfile)
    hpose.dump_pdb(hfile)
