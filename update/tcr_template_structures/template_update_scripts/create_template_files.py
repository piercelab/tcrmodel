...
import sys
sys.path.insert(0,'/www/tcrmodel/update/tcr_template_structures/template_update_scripts/')
...
if TAG == 'A':
    ...
    aho_cdr3_s = 107
    aho_cdr3_e = 138
    aho_cdr3extnd_s = aho_cdr3_s - 1
    aho_cdr3extnd_e = aho_cdr3_e + 1
    aho_hv4_s = 81
    aho_hv4_e = 90
...
for record in fasta_sequences:
    ...
    if check_tcr_alpha_beta_domain(record.seq) is True:
        ...
        with open(TAG+'_TCR_VD_ALL.fasta', 'a') as fr:
            fr.write(">"+outheader+"\n")
            fr.write(str(anarci_fseq)+"\n")
    else:
        print record.seq
    ...
    seq_vdomain,seq_gm,seq_fw,seq_cdr1,seq_cdr2hv4,seq_cdr3,regexres_e,seq_cdr3extnd="","","","","","","",""
    if anarci_fseq:
        ...
        if regexres:
            ...
            regexres_e =  assign_CDRs_using_REGEX_with_cdr3extnd(anarci_fseq, TAG)
            seq_cdr3extnd = regexres_e.groups()[6]   

            #print "cap", seq_vdomain, seq_fw, seq_cdr1, seq_cdr2hv4, seq_cdr3
            with open(TAG+'_TCR_VDOMAIN_ALL.fasta', 'a') as fr:
                fr.write(">"+outheader+"\n")
        ...
        if regexres:
            ...
            seq_cdr2hv4 = regexres.groups()[4]
            seq_cdr3 = regexres.groups()[6]   
            regexres_e =  assign_CDRs_using_REGEX_with_cdr3extnd(fseq, TAG)
            seq_cdr3extnd = regexres_e.groups()[6]   
            ...
            if regexres:
                ...
                seq_cdr2hv4 = regexres.groups()[4]
                seq_cdr3 = regexres.groups()[6]
                regexres_e =  assign_CDRs_using_REGEX_with_cdr3extnd(fseq, TAG, True)
                seq_cdr3extnd = regexres_e.groups()[6]   
                print "nocap", seq_vdomain, seq_fw, seq_cdr1, seq_cdr2hv4, seq_cdr3, seq_cdr3extnd
            else:
    ...
    #pdbgzfile = os.path.join("/home/gowthamanr/test/"+pdbid+".pdb.gz")
    ...
    if pdbregex:
        ...
        with open(TAG+'_TCR_VDOMAIN_clean.fasta', 'a') as fr:
            fr.write(">"+outheader+"\n")
            fr.write(seq_vdomain+"\n")
        ...
        with open(TAG+'_TCR_CDR3_'+str(len(seq_cdr3))+'.fasta', 'a') as fr:
            fr.write(">"+outheader+"\n")
            fr.write(seq_cdr3+"\n")
        with open(TAG+'_TCR_CDR3extnd_'+str(len(seq_cdr3extnd))+'.fasta', 'a') as fr:
            fr.write(">"+outheader+"\n")
            fr.write(seq_cdr3extnd+"\n")
        ...
        outfile = pdbid+"_"+chainid+"_"+TAG+"_cdr3extnd"+"_"+str(len(seq_cdr3extnd))+".pdb"
        extract_region(capahopose, aho_cdr3extnd_s, aho_cdr3extnd_e, chainid, outfile)
        cdrfile.write("cdr3extnd"+" "+seq_cdr3extnd+" "+pdbid+" "+chainid+" "+str(len(seq_cdr3extnd))+" "+domtag+" "+"OK"+"\n")
        ...
        pos_cdr3extnds = inpose.pdb_info().pdb2pose(chainid,aho_cdr3extnd_s)
        pos_cdr3extnde = inpose.pdb_info().pdb2pose(chainid,aho_cdr3extnd_e)
        pos_hv4s = inpose.pdb_info().pdb2pose(chainid,aho_hv4_s)
        pos_hv4e = inpose.pdb_info().pdb2pose(chainid,aho_hv4_e)

        if not seq_vdomain == "None":
            with open("other_"+TAG+'_TCR_VDOMAIN.fasta', 'a') as fr:
                fr.write(">"+outheader+"\n")
                fr.write(seq_vdomain+"\n")
        ...
        if not ( (int(pos_cdr1s) == 0) or (int(pos_cdr1e) == 0) ):
            ...
            if (str(seq_cdr1) == str(pdb_cdr1)):
                CDR1_match=True
                with open('other_'+TAG+'_TCR_CDR1_'+str(len(seq_cdr1))+'.fasta', 'a') as fr:
                    fr.write(">"+outheader+"\n")
                    fr.write(pdb_cdr1+"\n")
                outfile = pdbid+"_"+chainid+"_"+TAG+"_cdr1"+"_"+str(len(seq_cdr1))+".pdb"
        ...
        if not ( (int(pos_cdr2s) == 0) or (int(pos_cdr2e) == 0) ):
            ...
            if (str(seq_cdr2_aho) == str(pdb_cdr2)):
                CDR1_match=True
                with open('other_'+TAG+'_TCR_CDR2_'+str(len(seq_cdr2_aho))+'.fasta', 'a') as fr:
                    fr.write(">"+outheader+"\n")
                    fr.write(pdb_cdr2+"\n")
                outfile = pdbid+"_"+chainid+"_"+TAG+"_cdr2"+"_"+str(len(seq_cdr2_aho))+".pdb"
        ...
        if not ( (int(pos_cdr2s) == 0) or (int(pos_cdr2e) == 0) ):
            ...
            if (str(seq_cdr2hv4) == str(pdb_cdr2hv4)):
                CDR2HV4_match=True
                with open('other_'+TAG+'_TCR_CDR2HV4_'+str(len(seq_cdr2hv4))+'.fasta', 'a') as fr:
                    fr.write(">"+outheader+"\n")
        ...
        if not ( (int(pos_hv4s) == 0) or (int(pos_hv4e) == 0) ):
            ...
            if (str(seq_hv4_aho) == str(pdb_hv4)):
                HV4_match=True
                with open('other_'+TAG+'_TCR_HV4_'+str(len(seq_hv4_aho))+'.fasta', 'a') as fr:
                    fr.write(">"+outheader+"\n")
                    fr.write(pdb_hv4+"\n")
                outfile = pdbid+"_"+chainid+"_"+TAG+"_hv4"+"_"+str(len(seq_hv4_aho))+".pdb"
        ...
        if not ( (int(pos_cdr3s) == 0) or (int(pos_cdr3e) == 0) ):
            ...
            if (str(seq_cdr3) == str(pdb_cdr3)):
                CDR3_match=True
                with open('other_'+TAG+'_TCR_CDR3_'+str(len(seq_cdr3))+'.fasta', 'a') as fr:
                    fr.write(">"+outheader+"\n")
                    fr.write(pdb_cdr3+"\n")
                outfile = pdbid+"_"+chainid+"_"+TAG+"_cdr3"+"_"+str(len(seq_cdr3))+".pdb"
        ...
        CDR3extnd_match = False
        if not ( (int(pos_cdr3extnds) == 0) or (int(pos_cdr3extnde) == 0) ):
            pdb_cdr3extnd = pdbseq[pos_cdr3extnds-1:pos_cdr3extnde]
            if (str(seq_cdr3extnd) == str(pdb_cdr3extnd)):
                CDR3extnd_match=True
                with open('other_'+TAG+'_TCR_CDR3extnd_'+str(len(seq_cdr3extnd))+'.fasta', 'a') as fr:
                    fr.write(">"+outheader+"\n")
                    fr.write(pdb_cdr3extnd+"\n")
                outfile = pdbid+"_"+chainid+"_"+TAG+"_cdr3extnd"+"_"+str(len(seq_cdr3extnd))+".pdb"
                extract_region(inpose, aho_cdr3extnd_s, aho_cdr3extnd_e, chainid, outfile)
                cdrfile.write("cdr3extnd"+" "+seq_cdr3extnd+" "+pdbid+" "+chainid+" "+str(len(seq_cdr3extnd))+" "+domtag+" "+"OK"+"\n")
            else:
                cdrfile.write("cdr3extnd"+" "+seq_cdr3extnd+" "+pdbid+" "+chainid+" "+str(len(seq_cdr3extnd))+" "+domtag+" "+"Mismatch"+"\n")
                #print "cdr3", "Mismatch", pdb_cdr3, seq_cdr3
        else:
            cdrfile.write("cdr3extnd"+" "+seq_cdr3extnd+" "+pdbid+" "+chainid+" "+str(len(seq_cdr3extnd))+" "+domtag+" "+"Mismatch"+"\n")
            #print "cdr3", "Mismatch", pdb_cdr3, seq_cdr3            
        ...
        if not ( (int(pos_cdr1s) == 0) or (int(pos_cdr1e) == 0) or (int(pos_cdr2s) == 0) or (int(pos_cdr2e) == 0) or (int(pos_cdr3s) == 0) or (int(pos_cdr3e) == 0) ):
            ...
            if (str(seq_fw) == str(pdb_fw)):
                FW_match=True
                with open('other_'+TAG+'_TCR_FW.fasta', 'a') as fr:
                    fr.write(">"+outheader+"\n")
                    fr.write(pdb_fw+"\n")
        ...
        if not ( (int(pos_cdr1s) == 0) or (int(pos_cdr1e) == 0) or (int(pos_cdr2s) == 0) or (int(pos_cdr2e) == 0) or (int(pos_cdr3s) == 0) or (int(pos_cdr3e) == 0) ):
            ...
            if (str(seq_gm) == str(pdb_gm)):
                GM_match=True
                with open('other_'+TAG+'_TCR_GM.fasta', 'a') as fr:
                    fr.write(">"+outheader+"\n")
                    fr.write(pdb_gm+"\n")
